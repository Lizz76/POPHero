...
- 方块收益：命中的 `Block` 会贡献攻击、防御或倍率收益。
- 敌人接敌：近战敌人默认从 3 步外出生，逐步前进，贴脸后攻击。
- 飞行支援敌人：可由 `encounter.csv` 配置进遭遇组合；它不接敌，每回合从原点远程攻击。
- 双敌目标：玩家输出始终打离主角最近的存活敌人，伤害不溢出到另一只敌人。
- 遭遇规则：战斗敌人由 `encounter.csv` 的遭遇配置决定，同一个 encounter 组合固定，并按楼层范围进入抽取池。
...
- 基准：`100` 金币代表玩家已经明显攒钱，属于略富有状态。
- 普通战斗：金币是低额稳定收入，Act 1 普通遭遇总金币约为早期 `8-16`、中期 `18-20`、后期 `25-27`；非 Boss 单体敌人金币不超过 `24`。
- Boss 战：Boss 允许成为明显补给点，当前 Boss 金币为 `65`，但不代表普通小怪金币曲线。
- 商店消费：`100` 金币可以买 2-3 件普通物品，例如 Sticker 或方块，但基本只够买 1 件机制性遗物；当前 Mod 统一视为机制性遗物，价格为 `100`。
- 价格口径：Sticker `32`、方块 `42`、成长项约 `45-70`、删除方块 `45`、商店/奖励刷新 `12`。普通战斗 1-2 场不应足够清空一轮商店。
...
- 普通战斗敌人由 `encounter.csv` 决定，不依赖代码固定第一场/第二场组合。
- Act 1 普通遭遇池有前期/中期/后期分层，Floor 1 不会抽到后期高压组合，Floor 6-7 可以抽到后期组合。
- 含小鸟的 encounter 使用 `3901:slot_air`，场上应显示 bird prefab。
